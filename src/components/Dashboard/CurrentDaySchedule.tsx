...
import { ClipboardList, Shield, Eye, Calendar } from 'lucide-react';

interface CurrentDayScheduleProps {
  absentTeachers: { name: string; subject: string; periods: number }[];
  supervisors: string[];
  dutyStaff: string[];
}

const CurrentDaySchedule: React.FC<CurrentDayScheduleProps> = ({ absentTeachers, supervisors, dutyStaff }) => {
      ...
      <div className="flex items-center justify-between mb-4 border-b border-gray-100 pb-4">
        <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-[#e5e1fe] flex items-center justify-center">
                <Calendar className="h-5 w-5 text-[#655ac1]" />
            </div>
            جدول اليوم الحالي
        </h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 max-h-[250px]">
        {activeTab === 'absences' && (
          <>
            ...
            )) : <p className="text-center text-gray-400 py-4 text-xs">لا يوجد غياب اليوم</p>}
          </>
        )}
        ...
        {activeTab === 'supervisors' && (
          <>
             ...
             )) : <p className="text-center text-gray-400 py-4 text-xs">لا يوجد مشرفين اليوم</p>}
          </>
        )}
        ...
        {activeTab === 'duty' && (
          <>
             ...
             )) : <p className="text-center text-gray-400 py-4 text-xs">لا يوجد مناوبين اليوم</p>}
          </>
        )}
      </div>
    </div>
...
export default CurrentDaySchedule;
