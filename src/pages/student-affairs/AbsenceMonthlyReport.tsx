...
interface MonthlyAbsenceRecord {
  id: string;
  studentId: string;
  studentName: string;
  classRoom: string;
...
const AbsenceMonthlyReport = () => {
  ...
  // الأشهر الهجرية - كل 4 أسابيع = شهر دراسي
  const months = [
    { value: '1', label: 'الشهر الأول (الأسابيع 1-4)' },
    { value: '2', label: 'الشهر الثاني (الأسابيع 5-8)' },
    { value: '3', label: 'الشهر الثالث (الأسابيع 9-12)' },
    { value: '4', label: 'الشهر الرابع (الأسابيع 13-16)' }
  ];
  
  const totalStudents = 450;
  const schoolDaysInMonth = 20; // 4 أسابيع × 5 أيام

  // سجلات الغياب الشهرية التجريبية
  const monthlyRecords: MonthlyAbsenceRecord[] = [
    {
      id: '1',
      studentId: '2024001',
      ...
      weeks: [
        ...
        },
        {
          weekNumber: 2,
          absenceDays: [
            { date: '2024-01-08', day: 'الاثنين', absenceType: 'excused', recordedBy: 'محمد السعيد' }
          ]
        },
        {
          weekNumber: 3,
        ...
        },
        {
          weekNumber: 4,
          absenceDays: [
            { date: '2024-01-22', day: 'الاثنين', absenceType: 'unexcused', recordedBy: 'أحمد المحمدي' }
          ]
        }
      ],
      totalAbsenceDays: 4,
      ...
      weeks: [
        {
          ...
          absenceDays: [
            ...
            { date: '2024-01-04', day: 'الأربعاء', absenceType: 'unexcused', recordedBy: 'أحمد المحمدي' }
          ]
        },
        {
          weekNumber: 2,
          absenceDays: [
            { date: '2024-01-09', day: 'الثلاثاء', absenceType: 'unexcused', recordedBy: 'محمد السعيد' },
            { date: '2024-01-10', day: 'الأربعاء', absenceType: 'none', recordedBy: 'أحمد المحمدي' }
          ]
        },
        {
          weekNumber: 3,
          absenceDays: [
            { date: '2024-01-15', day: 'الاثنين', absenceType: 'excused', recordedBy: 'أحمد المحمدي' }
          ]
        },
        {
          weekNumber: 4,
          absenceDays: [
            { date: '2024-01-23', day: 'الثلاثاء', absenceType: 'unexcused', recordedBy: 'محمد السعيد' }
          ]
        }
      ],
      totalAbsenceDays: 6,
    ...
    },
    {
      ...
      weeks: [
        ...
        },
        {
          weekNumber: 2,
          absenceDays: [
            { date: '2024-01-08', day: 'الاثنين', absenceType: 'excused', recordedBy: 'أحمد المحمدي' }
          ]
        },
        {
          weekNumber: 3,
          absenceDays: [
            { date: '2024-01-16', day: 'الثلاثاء', absenceType: 'excused', recordedBy: 'محمد السعيد' }
          ]
        },
        {
          weekNumber: 4,
  ...
  const absentWithExcuse = filteredRecords.reduce((sum, record) => {
    return sum + record.weeks.reduce((weekSum, week) => {
      return weekSum + week.absenceDays.filter(d => d.absenceType === 'excused').length;
    }, 0);
  }, 0);
  const absentWithoutExcuse = filteredRecords.reduce((sum, record) => {
    return sum + record.weeks.reduce((weekSum, week) => {
      return weekSum + week.absenceDays.filter(d => d.absenceType === 'unexcused').length;
    }, 0);
  }, 0);
  const totalAbsenceDays = filteredRecords.reduce((sum, record) => sum + record.totalAbsenceDays, 0);
  const absenceRate = totalStudents > 0 ? ((totalAbsenceDays / (totalStudents * schoolDaysInMonth)) * 100).toFixed(1) : '0';
  ...
  return (
    <div className="space-y-6 font-kufi" style={{ direction: 'rtl' }}>
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">التقرير الشهري للغياب</h1>
      ...
      {/* شريط الإحصائيات */}
      <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
        <h2 className="text-lg font-bold text-gray-800 mb-4">إحصائية الغياب الشهري</h2>
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-[#4f46e5] to-[#6366f1] text-white">
              <tr>
                ...
                <th className="px-4 py-4 text-right text-sm font-bold">الطالب</th>
                <th className="px-4 py-4 text-right text-sm font-bold">الأسبوع 1</th>
                <th className="px-4 py-4 text-right text-sm font-bold">الأسبوع 2</th>
                <th className="px-4 py-4 text-right text-sm font-bold">الأسبوع 3</th>
                <th className="px-4 py-4 text-right text-sm font-bold">الأسبوع 4</th>
                <th className="px-4 py-4 text-right text-sm font-bold">الإجمالي</th>
                <th className="px-4 py-4 text-right text-sm font-bold">النسبة</th>
              </tr>
            </thead>
                  ...
                  <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-600">{record.classRoom}</p>
                        <p className="text-xs text-gray-500">{record.studentId}</p>
                      </div>
                    </td>
                    {record.weeks.map((week, idx) => (
                      <td key={idx} className="px-4 py-4">
                        {week.absenceDays.length > 0 ? (
                          <div className="space-y-1">
                            {week.absenceDays.map((day, dayIdx) => (
                              <div key={dayIdx} className="text-xs bg-gray-50 p-2 rounded">
                                <div className="font-medium text-gray-900">{day.day}</div>
                                <div className="text-gray-600">{new Date(day.date).toLocaleDateString('ar-SA', { month: 'short', day: 'numeric' })}</div>
                                <div className="mt-1">
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xl font-bold text-gray-400">لا توجد سجلات غياب لهذا الشهر</p>
                    </div>
                  </td>
                </tr>
              )}
